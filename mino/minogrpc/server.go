...
func (o *overlayServer) Stream(stream ptypes.Overlay_StreamServer) error {
	...
	endpoint.Lock()
	dela.Logger.Warn().Msgf("overlayServer.Stream [stream %v] - LOCKED", streamID)
	...
	if isRoot {
		...
		if err != nil {
			dela.Logger.Warn().Msgf("overlayServer.Stream [stream %v] - unlocking", streamID)
			endpoint.Unlock()
...
func (o *overlayServer) cleanStream(endpoint *Endpoint, id string) {
	...
	defer dela.Logger.Warn().Msgf("overlayServer.cleanStream [stream %v] - unlocking", id)
...
func (o *overlayServer) Forward(ctx context.Context, p *ptypes.Packet) (*ptypes.Ack, error) {
	dela.Logger.Warn().Msgf("overlayServer.Forward")
	defer dela.Logger.Warn().Msgf("overlayServer.Forward - DONE")
